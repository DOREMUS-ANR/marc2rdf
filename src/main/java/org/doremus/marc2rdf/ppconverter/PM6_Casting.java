...
public class PM6_Casting extends DoremusResource {
  private static final String SUB_INSTRUMENT_REGEX = "^[^(]+(\\(?dont ([^)\\n]+)\\)?)$";
  private static final Pattern SUB_INSTRUMENT_PATTERN = Pattern.compile(SUB_INSTRUMENT_REGEX);
  ...
  public PM6_Casting(String note, String uri) {
    super();
    this.uri = uri;
  ...
  private void parseNote(String note) {
    //  nothing to do
    if (note.equals("Instrumental")) return;
    ...
    if (note.startsWith("Rôles :")) return;
    ...
    if (note.contains(":") || note.contains(".")) return;
    ...
    for (String match : splits) {
      ...
      matcher = SUB_INSTRUMENT_PATTERN.matcher(part);
      if (matcher.find()) {
        ...
        for (String subMatch : subParts.split("\\+")) {
          ...
          String subPart = subMatch.trim()
            .replaceAll(" prenant l[ea]", "")
            .replaceAll(soloRegex1, "")
